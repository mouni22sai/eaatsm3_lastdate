...
var socket2 = socket_client('http://localhost:4000');
          ...
          client.getProperties(device.id,function(err,properties){
            if(err){
              ...
              if(properties["ro.build.product"]==="rimo01a"){
                $('#rimo01a').append('<li id="'+device.id.toString()+'"><h2>'+device.id.toString()+'</h2><button id="'+device.id.toString()+'-runtest" class="mtbf">Run Test</button><h3>Test Status : <span id="'+device.id.toString()+'-status"></span></h3></li>');
              }else if(properties["ro.build.product"]==="rimo02a"){
                $('#rimo02a').append('<li id="'+device.id.toString()+'"><h2>'+device.id.toString()+'</h2><button id="'+device.id.toString()+'-runtest" class="mtbf">Run Test</button><h3>Test Status : <span id="'+device.id.toString()+'-status"></span></h3></li>');
              }else if(properties["ro.build.product"]==="rimo03a"){
                $('#rimo03a').append('<li id="'+device.id.toString()+'"><h2>'+device.id.toString()+'</h2><button id="'+device.id.toString()+'-runtest" class="mtbf">Run Test</button><h3>Test Status : <span id="'+device.id.toString()+'-status"></span></h3></li>');
              }
              ...
              $('#'+device.id.toString()+'-runtest').on('click',function(){
                var child = fork('/home/'+username.trim()+'/Workspace/eaatsm3/MTBF/forked.js');
                ...
                child.on('message',function(msg){
                  ...
                  if(msg.stat==="test stopped"){
                    ...
                    var send = gmailsend({
                      ...
                      subject : "MTBF testing of "+msg.num.toString(),
                      text : "logfiles"
                    });
                ...
                var logchild = fork('/home/'+username.trim()+'/Workspace/eaatsm3/MTBF/logfork.js');
