...
var socket1 = socket_client('http://localhost:8000');
          ...
          client.getProperties(device.id,function(err,properties){
            if(err){
              ...
              if(properties["ro.build.product"]==="rimo01a"){
                $('#rimo01a').append('<li id="'+device.id.toString()+'"><h2>'+device.id.toString()+'</h2><input val="" id="'+device.id.toString()+'-packname"><button id="'+device.id.toString()+'-runtest" class="monkey">Run Test</button><h3>Test Status : <span id="'+device.id.toString()+'-status"></span></h3></li>');
                ...
                $('#rimo03a').append('<li id="'+device.id.toString()+'"><h2>'+device.id.toString()+'</h2><input val="" id="'+device.id.toString()+'-packname"><button id="'+device.id.toString()+'-runtest" class="monkey">Run Test</button><h3>Test Status : <span id="'+device.id.toString()+'-status"></span></h3></li>');
              }
              ...
              $('#'+device.id.toString()+'-runtest').on('click',function(){
                var child = fork('/home/'+username.trim()+'/Workspace/eaatsm3/monkey/forked.js');
                var testtype = $(this).attr('class');
                var pack = $('#'+device.id.toString()+'-packname').val();
                child.send({devId : device.id.toString(),testType : testtype,pname : properties["ro.build.product"],package : pack.toString(),sysname : username.trim()});
                child.on('message',function(msg){
                  ...
                  if(msg.stat==="test stopped"){
                    ...
                    var send = gmailsend({
                      ...
                      subject : "Monkey-testing of "+msg.num,
                      text : "logfiles"
                    });
                    console.log(msg.filespath);
                    var file = [msg.filespath.toString()+'/kernel.txt',msg.filespath.toString()+'/events.txt',msg.filespath.toString()+'/radio.txt',msg.filespath.toString()+'/main.txt'];
                    send({
                      files : file
                    }, function (err, res) {
                ...
                var logchild = fork('/home/'+username.trim()+'/Workspace/eaatsm3/monkey/logfork.js');
